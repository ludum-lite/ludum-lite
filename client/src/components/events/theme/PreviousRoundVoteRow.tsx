...
const StyledToggleButtonGroup = styled(ToggleButtonGroup).withConfig({
  shouldForwardProp: ignoreProps(['vote'])
})<
  StyledToggleButtonGroupProps
>`
  background: ${({ theme }) =>
    theme.themeColors.themeRoundVoting.buttonsBackground};
  padding: ${({ theme }) => theme.spacing(0.5)}px;

  ${({ vote }) =>
    (vote === -1 || vote === 0 || vote === 1) &&
    css`
      box-shadow: 0px 0px 0px 1px rgb(0, 0, 0, 11%);
    `}
`

interface StyledToggleButtonProps {
  isActive: boolean
  voteType: 'reject' | 'neutral' | 'approve'
}
const StyledToggleButton = styled(ToggleButton).withConfig({
  shouldForwardProp: ignoreProps(['isActive', 'voteType'])
